...
*Setup instructions for a gitian build of GIANT using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GIANT
...
Clone the git repositories for giant and gitian and then checkout the giant version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the giant repository
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/giant/.git/
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/giant-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/giant-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/giant-build.assert
```
