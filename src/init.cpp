...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GIANT Core is shutting down."));
...
#endif
    // Make sure only a single GIANT process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GIANT Core is probably already running."), strDataDir));
    ...
    //GIANT: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GIANT Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart GIANT Core to complete") << "\n";
        ...
        zwalletMain = new CzGICWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zGICWallet
        uiInterface.InitMessage(_("Syncing zGIC wallet..."));
