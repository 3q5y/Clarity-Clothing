...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // GIANT: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. GIANT can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(giant_locale);
    Q_INIT_RESOURCE(giant);
    ...
    /// 6. Determine availability of data directory and parse giant.conf
