...
#include "ui_zgiccontroldialog.h"
...
std::set<std::string> ZGICControlDialog::setSelectedMints;
std::set<CMintMeta> ZGICControlDialog::setMints;

bool CZGICControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZGICControlDialog::COLUMN_DENOMINATION || column == ZGICControlDialog::COLUMN_VERSION || column == ZGICControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZGICControlDialog::ZGICControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGICControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGICControlDialog::~ZGICControlDialog()
{
    delete ui;
}

void ZGICControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGICControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZGICControlWidgetItem* itemDenom(new CZGICControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZGICControlWidgetItem *itemMint = new CZGICControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zgicTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zgicTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zGIC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zGIC seed used to mint this zGIC is not the same as currently hold in the wallet";
            else
...
void ZGICControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGICControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGIC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGICControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGICControlDialog::GetSelectedMints()
{
...
void ZGICControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
