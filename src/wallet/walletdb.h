...
public:
    ...
    bool WriteZGICSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGICSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGICSeed_deprecated(uint256& seed);
    bool EraseZGICSeed();
    bool EraseZGICSeed_deprecated();

    bool WriteZGICCount(const uint32_t& nCount);
    bool ReadZGICCount(uint32_t& nCount);
