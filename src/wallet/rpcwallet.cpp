...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            HelpExampleCli("signmessage", "\"giantaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; giant server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GIC address");
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zgicTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zGIC ID", m.hashPubcoin.GetHex()));     // PubCoin
            ...
            if (m.hashStake == 0) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zgicTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zgicTracker->ListMints(true, true, true);
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zGIC is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zGIC
    bool fMinimizeChange = params[2].get_bool();    // Minimize change
    std::string address_str = params.size() > 3 ? params[3].get_str() : "";

    vector<CZerocoinMint> vMintsSelected;

    return DoZgicSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZgicSpend(nAmount, false, true, vMintsSelected, address_str);
}
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzGICTracker* zgicTracker = pwalletMain->zgicTracker.get();
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzGICTracker* zgicTracker = pwalletMain->zgicTracker.get();
    set<CMintMeta> setMints = zgicTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzGICTracker* zgicTracker = pwalletMain->zgicTracker.get();
    set<CMintMeta> setMints = zgicTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zgicTracker->Add(mint, true);
        count++;
...
UniValue setzgicseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
            "setzgicseed \"seed\"\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("setzgicseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzgicseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzGICWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzgicseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzgicseed\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("getzgicseed", "") + HelpExampleRpc("getzgicseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    ...
    CzGICWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzgicstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzgicstate\n"
    ...
    obj.push_back(Pair("dzgic_count", nCount));
...
void static SearchThread(CzGICWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZGIC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzgic(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzgic\n"
            ...
            "\nExamples\n" +
            HelpExampleCli("searchdzgic", "1, 100, 2") + HelpExampleRpc("searchdzgic", "1, 100, 2"));
    ...
    boost::thread_group* dzgicThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzgicThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzgicThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zgicTracker->GetSerialHashes());
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
...

    {
        ...
        while (nTries < 100) {
            ...
            if (fLocked) {
                if (zgicTracker->ClearSpendCache()) {
