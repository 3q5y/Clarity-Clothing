...
CzGICTracker::CzGICTracker(std::string strWalletFile)
{
...
CzGICTracker::~CzGICTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGICTracker::Init()
{
...
bool CzGICTracker::Archive(CMintMeta& meta)
{
...
bool CzGICTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGICTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGICTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGICTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzGICTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzGICTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzGICTracker::GetSerialHashes()
{
...
CAmount CzGICTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGICTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGICTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGICTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGICTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGICTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGICTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGICTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGICTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGICTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGICTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGICWallet* zGICWallet)
{
    bool iszGICWalletInitialized = (NULL != zGICWallet);
    CMintMeta meta;
    ...
    if (! iszGICWalletInitialized)
        zGICWallet = new CzGICWallet(strWalletFile);
    meta.isSeedCorrect = zGICWallet->CheckSeed(dMint);
    if (! iszGICWalletInitialized)
        delete zGICWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGICTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGICTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGICTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGICTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGICTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGICTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGICWallet* zGICWallet = new CzGICWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zGICWallet);
        }
        delete zGICWallet;
        LogPrint("zero", "%s: added %d dzgic from DB\n", __func__, listDeterministicDB.size());
    }
