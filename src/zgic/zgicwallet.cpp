...
CzGICWallet::CzGICWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgic seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGICSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGICSeed_deprecated()) {
                    LogPrintf("%s: Updated zGIC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgic seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgic wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGICWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGICCount(nCountLastUsed);
    else if (!walletdb.ReadZGICCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGICWallet::Lock()
{
    seedMaster = 0;
}

void CzGICWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGICWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGIC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGICWallet::LoadMintPoolFromDB()
{
...
void CzGICWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGICWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGICWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgicTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGICWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGIC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgicTracker which also adds to database
    pwalletMain->zgicTracker->Add(dMint, true);
...
void CzGICWallet::SeedToZGIC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGICWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGICWallet::GenerateDeterministicZGIC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGICWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGICWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGICWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
